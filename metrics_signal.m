function [average, max, min, rms, kurtosis, skewness]  = metrics_signal(x,print,fid)
  % x: vector of values
  ...
  if (print == 1)
    if (fid == 0)  % print to command window
       ...
       fprintf("RMS      : %10.3e\n",rms);
       fprintf("Min      : %10.3e\n",min);
       fprintf("Kurtosis : %10.3e\n",kurtosis);
       fprintf("Skewness : %10.3e\n",skewness);
    else
       fprintf(fid,"Max      : %10.3e\n",max);
       fprintf(fid,"Average  : %10.3e\n",average);
       fprintf(fid,"RMS      : %10.3e\n",rms);
       fprintf(fid,"Min      : %10.3e\n",min);
       fprintf(fid,"Kurtosis : %10.3e\n",kurtosis);
       fprintf(fid,"Skewness : %10.3e\n",skewness);
   endif
  endif
  endfunction
